...
def test_list_workspace_objects_paginates():
    pages = [
        {"objects": [{"object_id": 1, "object_type": "NOTEBOOK", "path": "/a"}], "next_page_token": "tk"},
        {"objects": [{"object_id": 2, "object_type": "DIRECTORY", "path": "/b"}], "next_page_token": ""},
    ]
...
def test_export_notebook_source_returns_decoded_text():
    import base64
    payload = {"content": base64.b64encode(b"# COMMAND ----------\nprint('hi')\n").decode(), "file_type": "SOURCE"}
    with patch("amx.db.adapters._databricks_workspace.requests.get") as g:
...
def test_list_jobs_yields_full_job_records_with_runs():
    ...
    get_resp = {"job_id": 11, "settings": {"name": "j1", "tasks": [{"task_key": "t1", "notebook_task": {"notebook_path": "/n"}}]}}
    runs_resp = {"runs": [{"run_id": 100, "state": {"result_state": "SUCCESS"}, "start_time": 1700000000000, "end_time": 1700000010000}], "has_more": False}
    with patch("amx.db.adapters._databricks_workspace.requests.get") as g:
        g.side_effect = [
