...
SCHEMA_DESCRIPTIONS: dict[str, dict[str, str]] = {
    ...
    "remote_notebooks": {
        "__table__": "Notebooks ingested from a remote platform (Snowflake or Databricks). Source is normalized to .ipynb JSON for unified handling.",
        "id": "AMX-internal autoincrement integer. Used as foreign-key target from remote_job_tasks.notebook_id_fk.",
        "profile_name": "Name of the AMX DB profile that owns this notebook (matches db_profiles.name).",
        "platform": "'snowflake' or 'databricks'. Determines which native id field (qualified_name vs. workspace_path) is populated.",
        "external_id": "Platform-native identifier. Databricks: Workspace object_id. Snowflake: fully-qualified DB.SCHEMA.NAME.",
        "name": "Human-readable notebook name (last path segment for Databricks; bare object name for Snowflake).",
        "workspace_path": "Databricks workspace absolute path (NULL for Snowflake notebooks).",
        "qualified_name": "Snowflake fully-qualified name DB.SCHEMA.NAME (NULL for Databricks notebooks).",
        "language": "Notebook language: python, sql, scala, r, or mixed (Databricks default magics permit mixing).",
        "source_text": "Normalized .ipynb JSON. For Databricks source-format notebooks this is the converted shape, not the raw # COMMAND ---------- text.",
        "source_hash": "sha256 of source_text. Used to short-circuit re-embedding when source has not changed since last ingest.",
        "last_modified_at": "When the notebook was last modified on the platform (TIMESTAMP, NULL if API does not expose it).",
    ...
    "remote_jobs": {
        "__table__": "Databricks jobs/workflows (Jobs API 2.x). One row per job; task graph in remote_job_tasks and recent runs in remote_job_runs.",
        "id": "AMX-internal autoincrement integer.",
        "profile_name": "Owning AMX DB profile.",
        "job_id": "Databricks job_id (INTEGER, unique per workspace).",
        "name": "Display name from Databricks.",
        "creator_user_name": "Databricks user who created the job.",
        "schedule_cron": "Quartz cron expression if a schedule is set (NULL for manual-only jobs).",
        "schedule_timezone": "IANA timezone for schedule_cron.",
        "schedule_pause_status": "PAUSED or UNPAUSED. PAUSED jobs do not auto-fire even if schedule_cron is set.",
        "max_concurrent_runs": "Concurrency cap from job settings.",
        "email_notifications_json": "JSON object with on_failure/on_success/on_start address lists.",
        "tags_json": "JSON object of Databricks job tags (key to value).",
        "last_run_status": "State result of the most-recent run (SUCCESS/FAILED/CANCELED/RUNNING).",
        "last_run_started_at": "Start time of the most-recent run.",
        "success_rate_30d": "Fraction (0.0..1.0) of runs in the last 30 days that succeeded. NULL when no runs fall in the window.",
        "ingested_at": "When AMX last refreshed this row.",
    },
    "remote_job_tasks": {
        "__table__": "Per-task definitions inside a remote_jobs row. Captures the task DAG and per-task type (notebook/python/sql/dbt/pipeline).",
        "id": "AMX-internal autoincrement integer.",
        "job_id_fk": "Foreign key to remote_jobs.id.",
        "task_key": "Databricks task_key, unique within a job.",
        "task_type": "notebook_task | python_wheel_task | sql_task | dbt_task | pipeline_task | spark_jar_task | etc.",
        "notebook_path": "Databricks workspace path of the executed notebook (only when task_type=notebook_task).",
        "notebook_id_fk": "Resolved foreign key to remote_notebooks.id (filled post-ingest by lineage pass).",
        "sql_query_id": "SQL warehouse query_id (only when task_type=sql_task).",
        "sql_warehouse_id": "SQL warehouse id this task runs against.",
        "pipeline_id_fk": "Foreign key to remote_pipelines.id (only when task_type=pipeline_task).",
        "depends_on_json": "JSON array of task_keys this task depends on within the same job.",
        "raw_definition_json": "Full task spec JSON as returned by Databricks Jobs API. Retained for audit and forward-compat with new task_types.",
    },
    "remote_job_runs": {
        "__table__": "Last N runs (default 20) of each job, kept for success-rate calculation and Studio display.",
        "id": "AMX-internal autoincrement integer.",
        "job_id_fk": "Foreign key to remote_jobs.id.",
        "run_id": "Databricks run_id.",
        "state_result": "Result state when the run terminated: SUCCESS | FAILED | CANCELED | RUNNING (latter for in-progress at ingest time).",
        "start_time": "Run start TIMESTAMP.",
        "end_time": "Run end TIMESTAMP (NULL while RUNNING).",
        "setup_duration_ms": "Setup phase duration in milliseconds.",
        "execution_duration_ms": "Execution phase duration in milliseconds.",
    },
    "remote_pipelines": {
        "__table__": "Databricks Delta Live Tables (DLT) pipeline definitions. One row per pipeline.",
        "id": "AMX-internal autoincrement integer.",
        "profile_name": "Owning AMX DB profile.",
        "pipeline_id": "Databricks pipeline_id (string UUID).",
        "name": "Pipeline display name.",
        "target_schema": "Default target schema for tables produced by the pipeline (NULL if not set).",
        "edition": "DLT product edition: CORE | PRO | ADVANCED.",
        "continuous": "0/1: whether the pipeline runs continuously vs. triggered.",
        "photon": "0/1: whether Photon engine is enabled.",
    ...
    "remote_streamlit_apps": {
        "__table__": "Snowflake STREAMLIT objects (Snowsight Streamlit apps).",
        "id": "AMX-internal autoincrement integer.",
        "profile_name": "Owning AMX DB profile.",
        "qualified_name": "Fully-qualified DB.SCHEMA.NAME of the STREAMLIT object.",
        "main_file": "Entry-point file name (e.g. streamlit_app.py).",
        "query_warehouse": "Warehouse the app queries against (NULL if not configured).",
        "root_location": "Stage path (@db.schema.stage/...) where the app source lives.",
        "owner": "Snowflake OWNER role on the STREAMLIT object.",
        "last_altered_at": "Last alteration TIMESTAMP per Snowflake metadata.",
        "ingested_at": "When AMX last refreshed this row.",
    },
    "remote_streams": {
        "__table__": "Snowflake CDC streams. Each row reads change-data from one source table.",
        "id": "AMX-internal autoincrement integer.",
        "profile_name": "Owning AMX DB profile.",
        "qualified_name": "Fully-qualified DB.SCHEMA.NAME of the STREAM object.",
        "source_table_fqn": "Fully-qualified name of the table this stream reads from.",
        "source_entity_id": "Resolved AMX catalog_entities.id for source_table_fqn (filled post-ingest; NULL if no matching entity).",
        "mode": "Stream mode: APPEND_ONLY | DEFAULT | INSERT_ONLY. Controls which change types the stream surfaces.",
        "stale_after": "TIMESTAMP after which the stream becomes stale and must be consumed (NULL if open-ended).",
        "owner": "Snowflake OWNER role on the STREAM object.",
        "ingested_at": "When AMX last refreshed this row.",
    },
    "remote_task_dependencies": {
        "__table__": "Snowflake task DAG edges. One row per parent->child task dependency. parent and child are FQNs of TASK objects. Composite PK on (profile_name, parent_task_fqn, child_task_fqn).",
        "profile_name": "Owning AMX DB profile.",
        "parent_task_fqn": "Fully-qualified DB.SCHEMA.NAME of the upstream TASK.",
        "child_task_fqn": "Fully-qualified DB.SCHEMA.NAME of the downstream TASK that runs after the parent.",
    },
    "remote_queries": {
        "__table__": "Saved queries (Databricks SQL) and recent query history (both platforms). kind discriminator drives the executed_at semantics.",
        "id": "AMX-internal autoincrement integer.",
        "profile_name": "Owning AMX DB profile.",
        "platform": "'snowflake' or 'databricks'.",
        "kind": "'saved' for user-saved queries; 'history' for execution records from QUERY_HISTORY / query-history endpoint.",
        "external_id": "Platform-native query identifier (Databricks query_id; Snowflake QUERY_ID).",
        "name": "Saved-query display name. NULL for kind='history'.",
        "sql_text": "Full SQL statement.",
        "sql_hash": "sha256 of sql_text. Used for change detection on saved queries and dedup on history.",
        "warehouse": "Warehouse the query ran on (NULL for saved queries that have no fixed warehouse).",
