...
class PlanningMixin:
    ...
    def _interpret_question_pass1(self, question: str) -> tuple[SearchPlan, dict[str, Any]]:
        ...
        system = (
            ...
            "- Broad missing-comment requests such as 'are any tables in our databases missing comments' are coverage_audit.\n"
            ...
            '- aggregation_op: "" | "max" | "min" | "top_k" | "bottom_k" | "count". Detect superlatives and rankings: most/least/highest/lowest/biggest/smallest/largest, top N, first/last, leading, bottom. Use "count" only for pure how-many questions.\n'
            '- aggregation_field: "" | "row_count" | "column_count" | "table_count". Pick the numeric facet the user is ranking by (rows = row_count; columns = column_count; tables = table_count).\n'
    ...
    def _review_question_plan_pass2(
        ...
        system = (
            ...
            "Detect superlatives/rankings (most/least/top-N/bottom-N). Set aggregation_limit=1 for superlatives, N for top-N, 0 if no aggregation.\n"
    ...
    def _align_answer_language(self, plan: SearchPlan, question_language: str) -> SearchPlan:
        """Force ``answer_language`` to English.

        AMX is English-only — every prompt asks for English output, every
        user-facing surface is English, and every deterministic answer
        composer renders English. The detector under ``_types.py`` still
        runs for telemetry but the plan always carries
        ``answer_language="english"`` regardless of input language so
        downstream paths never accidentally branch on the input locale.
        """
        normalized = "english"
        return SearchPlan(
            intent=plan.intent,
    ...
    def _align_plan_shape(self, plan: SearchPlan, question: str) -> SearchPlan:
        ...
        asks_count = any(token in sample for token in ("how many", "count"))
        asks_table_word = any(token in sample for token in ("table", "tables"))
        asks_column_word = any(
            token in sample for token in ("column", "columns", "field", "fields")
        )
        asks_listing = any(
            token in sample
            for token in (
        ...
        asks_comment_coverage = any(token in sample for token in ("comment", "comments"))
        asks_relationship = any(
            token in sample for token in ("join", "link", "relationship", "relate", "connect")
        )
