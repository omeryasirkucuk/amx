...
- `v0.10.10`: **Fix: "how is X uploaded / loaded / populated" was routing to `detect_scd_pattern`** (wrong tool — that answers history-retention, not load-mechanism). New system prompt rule routes load-mechanism questions to the data-side signals AMX actually has: `describe_table.analytics.last_modified` for last-write timestamp + `inspect_data_quality` on the main temporal column for first/latest record + cadence hint + explicit CDC-column callout (created_at + updated_at + deleted_at) when shaped that way + an explicit "direct ETL/orchestrator tap is v0.11 work" caveat. The rule explicitly bans `detect_scd_pattern` for this class. Same pattern as v0.10.3 "duplication" routing fix.
- `v0.10.9`: **Demoted noisy "Exact row count failed" WARNING to DEBUG.** The message bled through the live-display panel during `/ask` answers despite being a clean fallback (estimated row count is used). Same v0.10.1 cleanup pattern. Operators investigating slow counts can still get the line via `AMX_LOG_LEVEL=debug`.
- `v0.10.8`: **Column-shape signal for `detect_dimensional_role` — SAP-style schemas now classify correctly.** Reproducer: `vbrk` (SAP billing header, classic fact) returned "unknown" because it has no `fact_*` naming, no declared FKs, no partition, and dates are stored as varchar — leaving the v0.10.7 classifier with no fired signal. Fix: new column-shape pass counts measure-like (numeric + money/quantity name like `_amt`, `_value`, `netwr`, `mwsbk`, `fkimg`), ID-like (`_id`, `_key`, `_no`, plus SAP keys `mandt`/`vbeln`/`kunag`/...), and descriptive (varchar + label/name) columns. New rule: `≥3 measures + ≥4 ids + (temporal OR ≥10K rows)` → fact (medium confidence). Mirrors heuristic for dimensions with `≥5 descriptives + 0 measures`. The classifier now handles FK-free / opaque-naming enterprise schemas (SAP, Oracle eBusiness, peoplesoft) without losing the existing high-confidence path on Kimball-style names.
- `v0.10.7`: **`detect_dimensional_role` tool — fact / dimension / bridge / lookup / staging / audit / transactional classification, plus schema-wide star-vs-snowflake pattern detection.** User: "AMX'in 'bu şemanın ana tablosu nedir?', 'fact ve dim hangileri?', 'star schema mi snowflake mi?' sorularını yorumlaması lazım". One tool, two modes: pass `schema` + `table` for one classification; pass `schema` alone to rank EVERY table and infer the overall pattern (star_schema / snowflake_schema / flat / fact_only / unknown). Detection blends naming patterns (`fact_*` / `dim_*` / `stg_*` / `bridge_*` / `_history` / `_log`), structural signals (row-count percentile vs schema median, FK fan-out / fan-in, partition / clustering presence, temporal column), and dimension-to-dimension FK presence (snowflake vs star). Same `evidence` + `confidence` + `indicators` design pattern as `detect_scd_pattern` and `find_joinable_tables`.
- `v0.10.6`: **`detect_scd_pattern` tool — infer SCD type from data, not comments.** User: "AMX'in SCD tipleri (Type 1/2/3/4) hakkında soruyu cevaplaması lazım — yalnızca değişiklik aynı satırda mı, eski değerler yeni satır olarak mı, eski-yeni kolon olarak mı?" One-tool design. Detects: Type 2 temporal pair (`valid_from`/`valid_to`, `BEGDA`/`ENDDA`, `effective_*`, `start_date`/`end_date`); Type 2 current-flag (`is_current`/`is_active` filtered to bool/char(1) dtypes); Type 2 version/revision/seq_no; Type 3 paired columns (`prev_X`/`X`, `old_X`/`new_X`, `previous_*`/`current_*`); Type 4 companion table (`<X>_history`/`_hist`/`_audit`/`_log`/`_archive`/`_versions` in same schema); optional Type 1 vs 2 row-cardinality probe when `business_key` is provided. Returns `scd_type_hypothesis` + `confidence` + `evidence` (LLM ALWAYS quotes these) + `alternative_hypotheses` for hybrid Type 6 cases. System prompt routes "how does X hold history?" / "değişiklik tek satırda mı yeni satır mı" / "is this SCD2 mi" to this tool.
- `v0.10.5`: **New `sample_column_values` tool — cheap direct `SELECT col FROM schema.table LIMIT N`.** User: "give me a date example from `aedat` in `bkpf`". Agent picked the wrong schema (`public` instead of `sap_s6p`), `profile_table` failed, and the agent told the user to run `/search sync` (misleading — the catalog isn't the issue). v0.10.5 adds a lightweight tool that bypasses `profile_table` entirely (no FK scan, no full column profile) and returns just the requested column's distinct non-null values. System prompt rule: when the user asks for a sample/example AND didn't qualify the schema, call `find_table_by_name` FIRST so the agent doesn't blindly guess `public`. Only fall through to `/search sync` advice when there's no exact AND no fuzzy table-name match.
- `v0.10.4`: **`/ask "date related columns"` now finds varchar dates too.** Pre-v0.10.4 `_DTYPE_FAMILIES["date"]` was just `["date"]`, so SAP-style `erdat`/`audat`/`varchar(8)` dates were invisible. Now `date` / `timestamp` / `time` / `temporal` are full semantic buckets covering all native temporal dtypes AND a name-pattern query for varchar columns with date-like names (`*_date`, `*_dt`, `*_at`, `dat_*`, `erdat`, `audat`, `created_*`, `valid_from/to`, `begda`, `endda`, etc., restricted to string-family dtypes). Each result row carries a `kind` field — `native_temporal` or `name_inferred_temporal` — so the LLM can be honest about how the match was found. Same pattern as v0.9.8 boolean `flag_candidate` tagging.
- `v0.10.3`: **Interpretive answering for "duplication" + "update soon" questions.** Two more cases where the agent fell back to "I don't know" / "give me columns" instead of using the data it had. (1) "is there duplication in vbak" → pre-v0.10.3 the agent bounced back asking for columns; now `check_uniqueness` (when called with no columns + no PK) auto-runs `inspect_data_quality` and returns `duplicate_summary` + `likely_unique_columns` so the LLM proposes a candidate composite key on the first round. (2) "is there an update on vbak soon?" → pre-v0.10.3 returned "I don't know about future updates"; now the system prompt routes this to `describe_table.analytics.last_modified` and instructs the LLM to surface BOTH the last-known modification time AND the explicit limit ("scheduled future updates need an ETL / orchestrator tap, planned v0.11"). Same v0.9.11 interpretive-answering pattern: surface what's available, name the limit explicitly.
- `v0.10.2`: **Data-quality + uniqueness probes** — two new agent tools that hit the live DB to answer real-world analyst questions. `check_uniqueness(schema, table, columns?)` runs `COUNT(*) vs COUNT(DISTINCT)` to verify whether a column tuple is a true primary key, returning `total_rows / distinct_rows / duplicate_rows / uniqueness_ratio / is_unique`. `inspect_data_quality(schema, table, columns?)` returns per-column null/distinct counts + ratios, min/max, and `detected_format` for varchar columns whose samples look like dates (recognises ISO 8601, YYYY-MM-DD, YYYYMMDD, DD-MM-YYYY, DD/MM/YYYY, DD.MM.YYYY, etc. with a 60% confidence threshold). System prompt routes "PK duplicate oluyor mu?", "date format?", "ne zamandır tutuluyor?", "çoklama oranı?" to the right tool. Followup notes: datamart detection (naming heuristic) and join-cardinality estimation planned for v0.10.3; ETL refresh inference needs query-history tap (v0.11).
- `v0.10.1`: **Suppressed false-positive `LLM returned EMPTY content` warning during tool-calling rounds.** When `finish_reason=tool_calls`, an empty content body is the expected OpenAI-protocol shape (the actual call lives in `message.tool_calls`). The pre-v0.10.1 code warned anyway and surfaced "Check model name, API key, and provider dashboard." mid-stream of `/ask` answers — alarming despite being normal flow. Fix: warn only when `finish_reason` is NOT in `{tool_calls, function_call}`; tool-call rounds drop to DEBUG level. Genuine empty responses (stop / content_filter / length with no tool_calls) still warn loudly.
- `v0.10.0`: **Analytics-DB metadata extension.** AMX now extracts partition keys, clustering keys, storage format (native / parquet / delta / iceberg / external), storage size + file count, last-modified, table type, governance tags, PII columns, and indexes for every profiled table — across all 4 backends (PostgreSQL, Snowflake, BigQuery, Databricks). New `AnalyticsMetadata` dataclass on `TableProfile`; `describe_table` tool returns an `analytics` payload so the search agent can answer "is there a performance optimization opportunity here?", "when was X last updated?", "what format is `sales_fact`?", "is there any PII column in `finance` schema?", "is `vbak` partitioned?". Per-backend coverage matrix in CHANGELOG. Lineage + schema-evolution diffs noted as v0.11 followups (require query-history tap).
- `v0.9.11`: **Make the system more interpretive — never reply with a flat 'no'.** User: "make this more interpretive" after AMX kept saying "no table found" on partial name queries. `find_table_by_name` now ships `fuzzy_matches: [{path, match_kind}]` covering prefix / suffix / contains / `SequenceMatcher` fuzzy (ratio ≥ 0.7) — populated from both the live DB and the catalog. System prompt grew an "Interpretive answering" section that teaches the LLM to read every field of every tool response (`fuzzy_matches`, `dtype_summary`, `columns_by_dtype`, `inference_source`, `kind`) before deciding the answer is empty. So a query like "I only remember 'trog' from the table name" now surfaces `trogr` / `trogt` etc. as candidates instead of "no exact match".
- `v0.9.10`: **Stop the dtype-question whack-a-mole.** v0.9.7 / 0.9.8 / 0.9.9 each fixed a specific dtype false-negative ("which tables can I join", "any boolean column", "which int or double columns"). User's correct diagnosis: this is a design problem, not a bug — the LLM wasn't getting a complete dtype picture so each question class needed a separate patch. v0.9.10 adds `columns_by_dtype: {family: [column_names]}` to `describe_table` — complete coverage, never truncated, regardless of table width. Tool description tells the LLM: "for any dtype question, read `columns_by_dtype` and list the names; don't say 'no X' unless the family key is absent". Future dtype questions (any json, any uuid, any bytea) need no AMX-side change.
- `v0.9.9`: **Fix `describe_table` truncating before the LLM saw rare dtypes.** Reproducer: user asks "do vbak have any boolean column?" — `vbak`'s only `bool` column (`is_deleted`) was at position 155 of 200+ columns; `cols[:60]` truncated it before the LLM saw it, and the answer was "no boolean columns" with confidence. Three fixes: (1) new `dtype_summary` field counts ALL dtype families across the whole table (travels with the prompt regardless of truncation); (2) smart truncation order — commented columns first, then rare dtypes, then alphabetical, so `is_deleted` survives the cap; (3) new `columns_truncated` boolean for explicit honesty.
- `v0.9.8`: **Fix `/ask "is there any boolean column in vbak"` false-negative on SAP `char(1)` flag schemas.** SAP and many enterprise systems store boolean SEMANTICS as `char(1)` flags (`'X'`/`''` or `'Y'`/`'N'`) — pre-v0.9.8 `find_columns_by_dtype('boolean')` only matched literal `bool` PG dtypes so flag columns like `autlf`/`faksk`/`lifsk` were invisible. Now the boolean family includes `char(1)` / `varchar(1)`; each result carries a `kind` field (`native_boolean` vs `flag_candidate`) and the tool descriptions instruct the LLM to surface BOTH instead of saying "no boolean columns".
- `v0.9.7`: **Fix `/ask "which tables can I join with vbrk"` returning 0 results on FK-free schemas.** SAP / legacy DBs manage referential integrity at the application layer, so the pre-v0.9.7 `joinable_tables` (declared-FK only) found nothing and the LLM said "no joinable tables" — wrong. New `name_overlap_joinable_tables` heuristic finds candidates by shared column NAMES with inverse-log rarity scoring (so `mandt`-everywhere doesn't drown high-signal shared keys like `vbeln`). `find_joinable_tables` now uses a 3-tier fallback: declared FK → name-overlap → semantic similarity. The result includes `inference_source` so the LLM can state the tier honestly ("via the `vbeln` foreign key" vs "by shared column names" vs "by semantic similarity").
- `v0.9.6`: **Fix cascading mixin import gaps (`name 're' is not defined` and 22 siblings).** v0.9.5 fixed the dataclasses but missed the long tail of bare-name references — stdlib (`re`, `json`, `time`, `sqlite3`), typing helpers (`Callable`, `asdict`), and project types (`DatabaseConnector`, `LLMProvider`, `TableProfile`, `CodebaseReport`, `MetadataSuggestion`, `CodeReference`, `SearchAnswer`, `SequenceMatcher`, `step_spinner`). 15 imports added across 12 mixin files. New full-scope name-resolution audit reports 0 unresolved references (was 23). Followup: adopt the audit script as a pre-commit hook so future mixin extractions can't ship with this class of regression.
- `v0.9.5`: **Fix mixin regression (`name 'DEFAULT_SETTINGS' is not defined` and 16 sibling errors).** v0.9.0/v0.9.1 mixin splits left module-level names (`SearchPlan`, `_input_token_budget_for`, `DEFAULT_SETTINGS`, `SOURCE_PRIORITY` …) referenced by bare name from mixin method bodies — but each mixin was now its own module. Fix: two new shared modules `_agent/_types.py` and `_catalog/_constants.py` own the dataclasses + helpers + constants; `agent.py`, `catalog.py`, and every mixin import from there. Public API preserved (`from amx.search.agent import SearchPlan` still works via re-export). Reproducer (`/ask` after a clean install) now passes.
- `v0.9.4`: **`execute_analyze_run` extracted into 3 phase helpers (S4 refactor).** The `/run` entry point was a 600-LOC procedural script juggling 12 local variables across 3 exception handlers; three different bugs surfaced there in recent conversations. v0.9.4 lifts the three largest chunks into a new `amx/cli_support/commands/_analyze/` package: `run_loop.py` (per-schema orchestration loop), `run_summary.py` (post-loop summary + apply branch), `interrupt.py` (KeyboardInterrupt body decision). execute_analyze_run is now 390 LOC (-35%); analyze_flow.py: 1099 → 877 LOC. Each phase is independently testable; exception handlers no longer race the main path for shared variables.
- `v0.9.3`: **Slash commands collapsed into a single registry (S5 refactor).** Before, each command had to be listed in 4 places in `session.py` (autocomplete catalog, `*_cmd_heads` frozensets, namespace help, dispatch chain). v0.9.3 introduces `amx/cli_support/slash_commands.py` — a 301-LOC declarative registry of `SlashCommand` dataclasses; the autocomplete catalog (5-line adapter) + dispatch heads (single helper call) now derive from it. session.py: 1283 → 1125 LOC. Drift between sources is now structurally impossible. Side-effect: the `/tls` command, previously in autocomplete but missing from dispatch, is now correctly routed.
- `v0.9.2`: **`Orchestrator.process_table` god-method extracted into `TableProcessor` (S3 refactor).** The 281-LOC method grew four filter chains plus an agent loop plus three apply branches; each new feature (Phase 2 dedup, Column scope) layered another filter on top. v0.9.2 extracts the flow into `amx/agents/_orchestrator/table_processor.py` (12 phase methods, none over 77 LOC). `Orchestrator.process_table` is now 25 lines — a thin delegator. Public signature unchanged. Each filter is now unit-testable in isolation; the three apply branches no longer compete for context. orchestrator.py: 1719 → 1463 LOC.
- `v0.9.1`: **SearchCatalog god-class split into 6 mixin modules (S2 refactor).** `amx/search/catalog.py` was 2033 LOC with 53 methods (entity CRUD + sync + search + join + usage + settings). v0.9.1 applies the same mixin pattern as v0.9.0: split by responsibility, public API preserved via Python MRO. catalog.py is now 200 LOC (-90.2%); the new mixins live under `amx/search/_catalog/` (entity_crud, sync, search, join, usage, settings). Method count preserved (53 = 3 core + 50 mixin). Same wins as the SearchAgent split: cluster-isolated unit tests, additive changes don't risk neighbouring code, future storage migrations touch only `entity_crud.py`.
- `v0.9.0`: **SearchAgent god-class split into mixin modules (S1 refactor).** `amx/search/agent.py` was 3733 LOC with 70 methods spanning 6+ responsibilities. v0.9.0 splits it into 7 mixin files under `amx/search/_agent/` (answering, deterministic, planning, resolution, retrieval, session_memory, short_circuits) plus a thin facade. agent.py is now 767 LOC (-79.5%). Public API unchanged — `SearchAgent.ask()` resolves all 70 methods transparently via Python MRO. Each cluster is independently testable; cross-mixin dependencies are documented in mixin docstrings.
- `v0.8.7`: **Banner footer no longer duplicates the version.** v0.8.6's footer was `v0.8.6 • AI-inferred database descriptions`, but the version is also shown in the "AMX Interactive Session" info block right below the banner. Two adjacent panels stamping the same version was noise. Footer is now just the tagline; version stays in the session info block where it groups with Config / Database / LLM. Banner = identity, session info = runtime state.
- `v0.8.6`: **Startup banner cleanup.** Dropped the redundant "Metadata Extraction System" subtitle (was saying the same thing as the title two lines above), swapped asterisks for box-drawing brackets so the framing matches the grid art instead of mixing vector glyphs, added a `v0.8.6  •  AI-inferred database descriptions` footer line auto-pulled from `amx.__version__`. Three-tier cyan hierarchy (tagline → art → footer) for readability.
- `v0.8.5`: **`/run` Column scope — re-run AI inference on a single column.** User: "yalnızca tek 1 kolon için /run çalıştıramıyo muyuz". New 5th scope option (`Database / Schema / Asset / Column / Default`) drills schema → table → column and restricts the run to just that one column. Useful when one comment came out wrong (LLM picked the wrong meaning for `code`, say) and you want to regenerate without re-profiling the whole table. New `ScopeResult` dict subclass carries optional `column_overrides`; orchestrator.process_table consults it before missing-only / dedup filters. Equivalence pre-walk respects the override too. Existing scope shapes are unchanged plain dicts so other consumers keep working.
- `v0.8.4`: **Dedup question now precedes the scope picker.** User clarified the `/edit` pattern: the binary mode-selector is the FIRST runtime question, before any drill-down. v0.8.2 had moved dedup ahead of coverage and review_strategy but it still came after the analysis-scope + schema pickers. v0.8.4 hoists `ask_choice("Equivalence-class deduplication?", ["dedup", "per-column"])` to right after `_resolve_completion_mode` and BEFORE the `with command_display(...)` block — so it fires before the scope picker. Profile/LLM-test/completion-mode prompts stay (infrastructure, not run-mode questions). New order: profile → LLM test → completion mode → **dedup** → scope → coverage → review strategy.
- `v0.8.3`: **Fix `UnboundLocalError: cannot access local variable 'review_strategy'` on Ctrl+C / blank-input cancellation at the schema picker.** Pre-existing bug surfaced by the user when they cancelled out of the asset-scope schema list during a `/run`: the KeyboardInterrupt handler reads `review_strategy` (and now `use_dedup` / `dedup_outcome`) before they're assigned by the runtime questions inside the `with command_display(...)` block. Now those variables are pre-initialised at the top of `execute_analyze_run` alongside the other early defaults, so the cancellation path finalises history cleanly without a secondary Python traceback. The dedup question + analysis panel from v0.8.2 still runs after scope is confirmed.
- `v0.8.2`: **Dedup is the FIRST runtime question + new equivalence analysis panel.** User: "ask the dedup Y/N in the same order as /edit". v0.8.1 still placed it after coverage and review_strategy; v0.8.2 moves it to the very first question after the scope picker (mirroring `/metadata edit`'s rule that the binary mode-selector always comes first). Order is now: scope → dedup → coverage → review_strategy. After picking `dedup` AMX walks the scope and shows an "Equivalence analysis" panel (heading + numbers + a small table of the top 10 classes that will dedup) — same pattern as `/metadata edit`'s "Bulk-update analysis for 'X'" header.
- `v0.8.1`: **Dedup question asked upfront, before scope walking.** User said "ask this first, like the /metadata edit pattern". v0.8.0 walked the scope, computed classes, showed a summary, then asked Y/N — too much work happened before the user could opt out. Now the dedup choice is a regular `ask_choice("Equivalence-class deduplication?", ["dedup", "per-column"])` asked alongside coverage (missing-only / all) and review strategy (individual / deferred / auto-apply), BEFORE any scope walking. Picking `per-column` skips the pre-walk entirely and goes straight to the legacy per-table flow.
- `v0.8.0`: **Equivalence-class deduplication for `/run` & `/run-apply` (Phase 2).** Wide schemas (think SAP) repeat the same column hundreds of times — `mandt`, `client`, `created_at`, `customer_id` — and the per-column LLM cost adds up. AMX now collapses these into equivalence classes before the per-table ProfileAgent loop runs: one LLM call per class with all member tables in the prompt as context, then the same description fans out to every member. The pre-walk respects the `missing-only` filter, the LLM can respond with `DIVERGES` to opt out of any class where the meaning genuinely differs, and the dedup pass is gated on a `Use equivalence-class deduplication for this run? (Y/n)` prompt that shows the estimated saving up front (`Found 145 columns → 12 classes; estimated 91.7% fewer column-level prompts`). End-of-run summary recaps how many classes were applied, how many diverged, and how many failed. New module `amx/agents/equivalence.py` (data + logic) + `amx/agents/equivalence_agent.py` (LLM pass). `Orchestrator.dedup_skip_set` filters dedup'd columns out of `process_table` so they don't get re-profiled. New `SearchCatalog.record_dedup_decision()` records each member with `source_kind='dedup'` linked to the run id.
- `v0.7.3`: **Bulk-edit picks the entity from the live DB instead of forcing the user to type a name + fix `info` `NameError` regression from 0.7.2.** User said: "Why am I typing the name? Let me PICK from a list, and let me drill down to column level — find similar to the asset I select." After choosing `Bulk by name`, the wizard now offers three sub-modes: `Pick a column from the catalog` (drills DB profile → schema → table → column, then uses the picked column's NAME so AMX bulk-fans-out to every other column with the same name across schemas — the default since column-level bulk is the most common case), `Pick a table from the catalog` (drills DB profile → schema → table, fans out by table name), or `Type a name manually` (legacy text-entry for power users). Confirmation line shows which name is being fanned out before the match table. Also fixes the v0.7.2 regression where `/metadata edit` → `Bulk by name` crashed with `name 'info' is not defined` because the new bulk-update analysis header used `info` without importing it.
- `v0.7.2`: **`/metadata edit` wizard asks bulk-vs-individual at the FIRST step.** User said: "I want the bulk option BEFORE 'What do you want to edit?'". Wizard now opens with `Single entity` (existing database → schema → table → column flow) vs `Bulk by name` (type a column or table name, AMX handles every match across schemas). New "Bulk-update analysis" header before the match table makes the impact explicit: "12 column(s) across 5 schema(s): sap_s6p, sap_test, … — whatever you select below will be updated TOGETHER with the same comment." Per user's request: "show me a simple analysis — these columns in these tables will all be updated."
- `v0.7.1`: **`/metadata edit <name>` asks bulk-vs-individual before locking you into bulk.** v0.7.0 went straight to the multi-select picker; that's wrong when entities just happen to share a name (`code` in `country.code` vs `currency.code` — different meanings). New three-way prompt after the match list: `bulk` (one comment for selected rows — original 0.7.0 behavior), `individual` (walk through each match one at a time, different comment per row, Enter to skip), or `cancel`. Single-match cases auto-skip the prompt and run the regular single-target edit. The `_run_individual_edits` loop writes via `apply_comment`, re-syncs to the catalog via `record_manual_description`, and reports `applied / skipped / failed` counts at the end.
- `v0.7.0`: **`/metadata edit <name>` now bulk-edits across every table+column matching the bare name.** Wide SAP-style schemas repeat column names (`mandt`, `client`, `customer_id`) in dozens of tables; until now the user had to type `/metadata edit <db>.<schema>.<table>.<column>` once per occurrence. Now `/metadata edit customer_id` searches the catalog for every match, prints a numbered table (kind / Schema.Table[.Column] / dtype / existing comment), asks for a multi-select picker (`1,3,5 / 1-4 / all`), takes ONE comment, and writes it via batched `COMMENT ON …` SQL to every selected entity. Catalog re-syncs so `/ask` sees the new descriptions immediately. User-curated multi-select keeps semantically-different tables that share a name from getting the same description by accident. New `SearchCatalog.find_columns_by_exact_name` underpins the lookup. Phase 2 — equivalence-class dedupe BEFORE the LLM call in `/run` — is planned as a follow-up release.
- `v0.6.4`: **Tool agent now filters concept-search false positives + handles push-back without parroting.** User asked "which tables have phone-number columns", agent returned `addrnumber`, `consnumber`, `persnumber`, `roomnumber` alongside `tel_number`/`fax_number` — the raw `search_columns_by_concept` candidate set. When the user said "some are not correct", agent thanked them and repeated the same list. Two prompt-level fixes: (a) explicit "Result validation" rule telling the model that `search_*_by_concept` returns a candidate set with false positives and it MUST drop rows whose description doesn't fit before answering, (b) "Push-back handling" rule with concrete next-step options (re-query with refined keyword, drill into descriptions, or admit limitation). Tool descriptions now state inline that results are a "CANDIDATE SET" so the model doesn't rediscover this each time.
