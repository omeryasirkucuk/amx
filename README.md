...
| `/search` + `/compare [RUN_IDS…] [--schema …] [--table …] [--column …] [--last N] [--command analyze.run\|search.ask\|all] [--by auto\|llm_profile\|doc_profile\|code_profile\|llm_model\|db_profile] [--diff] [--csv FILE] [--md FILE]` | Pivot two or more runs side-by-side. Three Rich tables: run summary (highlights the dimension that varies), per-column descriptions (top alternative + confidence band + `logprob_score` + tokens, with the best logprob per row in green), and aggregate metrics (duration, tokens, avg logprob, confidence distribution, approval rate). Use it to evaluate how a different LLM profile, doc profile, or code profile changed the descriptions you got for the same assets. `--diff` adds word-level highlights vs the leftmost run; `--csv` / `--md` also write the comparison to disk for sharing or charting. |
