...
- `Unreleased`: First-run UX and release-engineering baseline. Fresh installs no longer silently create a broken `localhost / amx_pass / SAP` "default" DB profile — the banner tells you to run `/setup`. Added `--debug` flag, themed top-level crash handler (no more raw tracebacks), and `chmod 0o600` on `~/.amx/config.yml` so saved passwords/API keys are owner-only. Plus the GitHub Actions CI/release pipeline, `python-semantic-release` for Conventional-Commit-driven versioning, pre-commit hooks (ruff, gitleaks), expanded PyPI metadata, and `CONTRIBUTING.md` / `SECURITY.md` / `LICENSE`. Run `pip install -e ".[dev]"` to pick up the dev tooling.
