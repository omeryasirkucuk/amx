...
- `Unreleased`: First-run UX, OS-keyring secret storage, and release-engineering baseline. DB passwords / LLM API keys / Databricks tokens are now stored in the OS keyring (macOS Keychain, Windows Credential Manager, Linux Secret Service); the YAML keeps only opaque `keyring:...` references. Existing plaintext configs are migrated automatically on the next save. Fresh installs no longer silently create a broken `localhost / amx_pass / SAP` "default" DB profile — the banner tells you to run `/setup`. Added `--debug` flag, themed top-level crash handler (no more raw tracebacks), and `chmod 0o600` on `~/.amx/config.yml`. Plus the GitHub Actions CI/release pipeline, `python-semantic-release` for Conventional-Commit-driven versioning, pre-commit hooks (ruff, gitleaks), expanded PyPI metadata, and `CONTRIBUTING.md` / `SECURITY.md` / `LICENSE`. Run `pip install -e ".[dev]"` to pick up the dev tooling.
